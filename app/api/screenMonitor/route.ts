...
export async function GET(req: NextRequest) {
  ...
  var userInfo = await getDoc(doc(db, "users", userId)).then((doc) =>
    doc.data()
  );
  ...
  if (hoursInuseage >= 6) {
    ...
    await addDoc(notificationsSubcollectionRef, {
      message: "Excess Screen Time of 6 Hour",
    ...
    await addDoc(notificationsSubcollectionRef, {
      message: "Running high,  3 Hour of screen time ,slow down",
      timestamp: serverTimestamp(),
    });
  }
