...
export function calculateScreenOnTime(
  ...
  for (let userIndex = 0; userIndex < allUsers.length; userIndex++) {
    ...
    if (screenTime / 1000 < minScreenTime * 60 * 60) {
      userScreentime.push({
        ...
        currentUserParent: [...currentUserParent.docs.map((p) => p.id)],
      });
    }
  }
...
export function formatDuration(a: Timestamp, b: Timestamp): string {
  var durationMillis = b.toDate().getTime() - a.toDate().getTime();
